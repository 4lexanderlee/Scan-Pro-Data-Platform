        ...
        <div className="relative z-10 max-w-6xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-8 backdrop-blur-sm">
            ...
            <span className="text-sm text-muted-foreground">
              Plataforma No-Code de análisis de datos · 100% gratuita
            </span>
          </div>
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-6 bg-gradient-to-r from-foreground via-primary to-secondary bg-clip-text text-transparent leading-tight">
            Explora, limpia y analiza
            <br />
            tus datos sin código.
          </h1>

          <p className="text-xl md:text-2xl text-muted-foreground mb-10 max-w-3xl mx-auto leading-relaxed">
            La plataforma gratuita que traduce tus clics a consultas{" "}
            <span className="text-foreground font-semibold font-mono">SQL</span>.{" "}
            Conecta tus archivos, haz visualizaciones y descubre insights al instante.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto">
            {[
              { icon: Database, label: "CSV & Excel", desc: "Ingesta de datos" },
              { icon: Wand2,    label: "Limpieza", desc: "Un clic" },
              { icon: Link2,   label: "Visual Joins", desc: "Sin código" },
              { icon: Terminal, label: "SQL Live", desc: "Motor educativo" },
            ].map((item, i) => (
              <div
                key={i}
